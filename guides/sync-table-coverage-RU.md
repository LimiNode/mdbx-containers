...
| `MetadataTable` | Поддержан | `Put`, `Delete`; захватываются все typed setter-ы, `set_schema_version()` и `erase()`. | String key и tagged value bytes replay-ятся в один DBI. Typed getter-ы после apply сохраняют fail-closed проверку типа и payload. | `test_sync_capture`, `test_sync_replication` |
