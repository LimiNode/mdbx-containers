...
Пока scoped baseline не реализован, новый scope можно активировать на его
назначенном origin с правом записи только тогда, когда все уже существующие DBI
из manifest пусты. Иначе registration завершается ошибкой о необходимости
baseline, а не объявляет пустую scoped history успешной сходимостью. Для уже
зарегистрированного scope эта проверка пустоты при повторной верификации не
выполняется; non-designated origin может установить descriptor поверх
существующих данных реплики только для включения локального write guard.
