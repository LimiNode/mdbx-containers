...
| `MetadataTable` | Supported | `Put`, `Delete`; every typed setter, `set_schema_version()`, and `erase()` are captured. | String keys and tagged value bytes are replayed into one DBI. Typed getters preserve their fail-closed type and payload validation after apply. | `test_sync_capture`, `test_sync_replication` |
...
| `KeyMultiValueTable<K, V>` | Schema-v3 logical adapter plus automatic capture | No raw `ChangeOp` in v0.1. | `SyncEngine::bind_key_multi_value_logical_capture()` durably binds the DBI to a receiver-neutral logical dataset. Ordinary supported API calls atomically add schema-v3 frames to `LogicalJournalStore`; routing later materializes each receiver route. Missing runtime binding, public raw-capture suppression, legacy typed capture sessions, and caller-created writable raw transactions fail closed. The normal unbounded `erase_range()` remains unsupported for a bound DBI; general multi-writer destructive convergence remains deferred. | `test_key_value_logical_adapter`, `test_sync_capture` negative coverage |
| `KeyOrderedMultiValueTable<K, V>` | Schema-v1 automatic append capture plus limited ordered adapters | No raw `ChangeOp` in v0.1. `SyncEngine::bind_key_ordered_multi_value_logical_capture()` binds schema v1 to a receiver-neutral logical journal; schema v2 captures `AppendElement` and exact `EraseElement` by persistent id. | Both schemas require one authoritative ordered origin and fail closed for direct logical frames or unordered delivery. A schema-v1 automatic binding journals ordinary `append`/`insert` calls, including batch append, and deliberately rejects destructive table calls; receivers are selected later through journal materialization. Schema v2 persists element identity and tombstones, and its typed capture atomically commits local mutations plus an ordered outbox envelope. Bounded `erase_at`, key/value erase, and clear resolve selectors to exact ids before mutation; `replace_with()` is also implemented for single-origin schema-v2 capture. The default resolver performs full reverse validation, while an opt-in transaction-bound proof uses a non-reusable session token, bounds its complete materialized ID set, and can reuse that validated set for trusted selector calls with a separate budget. Baseline import, multi-origin histories, and tombstone pruning/compaction remain separately deferred. | `test_key_value_logical_adapter`, `test_key_ordered_multi_value_destructive_state`, `test_key_ordered_multi_value_destructive_adapter` |
...
append-only and can be durably bound to receiver-neutral automatic journal
capture through `SyncEngine::bind_key_ordered_multi_value_logical_capture()`.
The binding records the logical dataset destination, never a receiver route;
ordinary `append`/`insert` calls (including batch append) publish schema-v1
frames atomically, while destructive table calls fail closed. Schema v2 adds
`AppendElement` and exact `EraseElement` by a
