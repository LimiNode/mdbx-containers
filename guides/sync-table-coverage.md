...
| `KeyOrderedMultiValueTable<K, V>` | Ordered logical adapter pending | No raw `ChangeOp` in v0.1. | Append-only logical changes require one ordered origin stream; direct logical frames and unordered delivery must fail closed. Destructive operations remain deferred pending persistent element identity and tombstones. | Ordered adapter round-trip and capture regressions |
