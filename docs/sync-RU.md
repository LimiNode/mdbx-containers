...
| --- | --- | --- |
| `KeyValueTable`, `KeyTable`, `ValueTable`, `SequenceTable`, `MetadataTable` | Поддерживается | Операции захватываются как физические изменения DBI. `MetadataTable` сохраняет записанные теги типов. |
