...
sequenceDiagram
    ...
    participant Stage as Подготовка получателя
...
### Подключаемое сохранение и возобновление полного импорта

`FullSnapshotImportOptions::persist_complete_staging = true` сохраняет принятые
нефинальные страницы `CompleteUserDatabase` в одном лениво создаваемом
зарезервированном DBI подготовки. После перезапуска процесса включите ту же
опцию, получите `SyncEngine::full_snapshot_import_resume()` и продолжите с
возвращёнными без изменений `snapshot_id` и continuation. Подключаемый fallback
полного снимка в `SyncWorker` делает это автоматически.

Срок хранения сессии по-прежнему определяет источник. Если источник возвращает
`SnapshotSessionInvalid`, worker удаляет постоянную подготовленную сессию, а
следующий fallback начинает новый снимок. Приложение может явно отказаться от
сессии через `discard_full_snapshot_import()`.
Начало нового несохраняемого импорта `CompleteUserDatabase` или явное
выключение сохранения подготовки также отбрасывает прежнюю постоянную сессию.

Финальная страница одной MDBX-транзакцией применяет пользовательские DBI,
инициализирует `_mdbxc_applied` и удаляет DBI подготовки. Поэтому прерывание до
этого commit не меняет пользовательские DBI, а после него не оставляет данных
для возобновления. Опция занимает один слот именованного DBI только пока импорт
не завершён. `ManifestOnly` и восстановление с учётом логического состояния в
этой v1 сохраняют прежний контракт подготовки в памяти.

### Проект: сохраняемое возобновление логического восстановления

Следующее расширение восстановления — отдельный durable-протокол для пути
`LogicalRecoveryRequest`. Это контракт проектирования, а не обещание текущего
релиза. Сохраняемая сессия должна явно содержать вид протокола
...
страницы и непрозрачное continuation, а также версию durable-формата/схемы.
Принятая страница и метаданные, описывающие точное следующее continuation,
фиксируются одной MDBX-транзакцией. Пользовательские DBI не изменяются до
финальной страницы.

Для logical-aware сессии финальная фиксация также восстанавливает logical
baseline: schema markers, replay markers и pruning watermark, frontiers
упорядоченной доставки и адресованный этому receiver суффикс ожидающих
исходящих envelope. Записи source outbox никогда не копируются в локальный
outbox receiver. Baseline, физическая замена, bootstrap raw-курсора и удаление
staging устанавливаются одной атомарной транзакцией.

После перезапуска новый engine должен восстановить ту же logical-сессию и
продолжить с неизменными `snapshot_id` и continuation. Новая source-сессия
допустима только после явного удаления durable-состояния либо если source
сообщил об истечении/недействительности сессии. Отмена или временный сбой
транспорта сохраняют возобновляемый staging; явный discard и окончательная
инвалидация сессии его удаляют.

Возобновление работает fail-closed при несовпадении вида протокола,
source/requester/DB, scope, manifest, tail, continuation, adapter/schema или
...
записи, они должны быть ограниченными и версионированными, а не порождать
неограниченный набор DBI на сессию. Acceptance-тесты должны прервать импорт
после нефинальной logical-страницы, перезапустить engine, возобновить ту же
сессию, применить baseline ровно один раз и доказать fail-closed для
повреждённого состояния, отмены, повторной финальной доставки и смешения
raw/logical-сессий.
