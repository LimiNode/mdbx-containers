...
- v0.1 захватывает обычные пути записи `KeyValueTable`, `KeyTable`,
  ...
  внешней сериализации пишущих узлов; разрешение конфликтов нескольких пишущих
  узлов не входит
  в контракт.
- `AnyValueTable`, `KeyMultiValueTable`, `KeyOrderedMultiValueTable` и
  ...
  `append()` по схеме v1 для одного назначенного origin. Его разрушающий
...
- `ConflictPolicy::LastWriterWins` имеет узкий путь «побеждает версия источника»
  ...
  метаданными sync, а служебные постоянные структуры хранят tombstone вместе с
  их версиями и записи регистрации. Полные снимки могут охватывать только manifest из raw DBI,
...
реплике. У коллекции должен быть один назначенный пишущий узел либо приложение
должно внешне сериализовать все пишущие узлы: локальный `add()` выделяет id из
