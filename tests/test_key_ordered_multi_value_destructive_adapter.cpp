...
void test_destructive_capture_key_index_proof_rejects_corruption() {
    const std::string path =
        "test_key_ordered_multi_value_destructive_key_index_proof_corrupt.mdbx";
    const std::string primary = "ordered_proof_corrupt_values";
    const std::string state = "ordered_proof_corrupt_state";
    const std::string by_key = "ordered_proof_corrupt_by_key";
    const std::string schema = "app.ordered_proof_corrupt.v2";
    cleanup(path);
    ...
    const std::shared_ptr<mdbxc::Connection> connection =
        mdbxc::Connection::create(config);
    const mdbxc::sync::NodeId origin = make_node(0x6Au);
    const mdbxc::sync::DbId local_db = make_node(0xAAu);
    const mdbxc::sync::DbId destination = make_node(0xBAu);
    mdbxc::sync::SyncEngine engine(connection);
    engine.initialize_local_identity(origin, local_db);
    table_type table(connection, primary);
    adapter_type adapter(table, schema, state, by_key);
...

    {
        mdbxc::Transaction transaction =
            connection->transaction(mdbxc::TransactionMode::WRITABLE);
        const std::vector<std::uint8_t> key_bytes = IntKeyCodec::encode(1);
        const std::vector<mdbxc::sync::OrderedElementId> ids =
            adapter.state_store().live_ids_for_key(
                transaction.handle(), key_bytes);
        if (ids.size() != 1u) {
            throw std::runtime_error(
                "ordered proof corruption fixture did not find index entry");
        }
        MDBX_dbi by_key_dbi = 0;
        mdbxc::check_mdbx(mdbx_dbi_open(
            transaction.handle(), by_key.c_str(),
            static_cast<MDBX_db_flags_t>(0), &by_key_dbi),
            "ordered proof corruption index open failed");
        const std::vector<std::uint8_t> index_value =
            mdbxc::sync::encode_ordered_element_id_index(ids[0]);
        MDBX_val raw_key = make_raw_val(key_bytes);
        MDBX_val raw_value = make_raw_val(index_value);
        mdbxc::check_mdbx(
            mdbx_del(transaction.handle(), by_key_dbi, &raw_key, &raw_value),
            "ordered proof corruption index delete failed");
        transaction.commit();
    }
    ...
    bool rejected = false;
    try {
        (void)session->validate_key_index(1, { 2u, 32u });
    } catch (const std::runtime_error&) {
        rejected = true;
    }
    if (!rejected || table.find(1).size() != 1u) {
        throw std::runtime_error(
            "ordered key index proof accepted a corrupted state");
    }
    bool reuse_rejected = false;
    try {
    ...
    if (!reuse_rejected) {
        throw std::runtime_error(
            "corrupted ordered proof session remained active");
    }
